...
# forget first 10 classes (0-9) for evaluation as OOD; training remains full data
...
lora_target=both
# per-batch forget/retain composition: none|balanced|proportional (keep none for full-data training)
batch_forget_mode=none
# where to save adapter only (PEFT uses a directory). Use a new directory name to avoid file conflicts.
adapter_path=checkpoints/${id}-${backbone}-${method}-normal-${lora_target}_adapter_peft



# tag method for evaluation to avoid overwriting outputs (embed key hparams)
method_tag=${method}-lt${lora_target}-normal-lora_r${lora_r}a${lora_alpha}d${lora_dropout}
