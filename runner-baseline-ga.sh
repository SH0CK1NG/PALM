...
# 保存路径（会覆盖完整模型权重）
# save_ckpt=checkpoints/${id}-${backbone}-${method}-baseline_ga_forget.pt
score="mahalanobis"
# method_tag=${method}-baseline-ga-b${batch}-e${epochs}-lr${lr}-wd${wd}-fl${forget_lambda}
method_tag=${method}-baseline-ga-b${batch}-e${epochs}-lr${lr}-wd${wd}-fl${forget_lambda}-${id}forget${forget_class_num}
...
# 用法：
# CUDA_VISIBLE_DEVICES=0 nohup bash runner-baseline-ga.sh > logs/runner_ga_$(date +%F_%H%M).log 2>&1 &
