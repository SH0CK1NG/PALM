...
# PyCIL EWC: CIFAR-80 -> CIFAR-110 (increment 5) + per-stage OOD eval
...
CONFIG_PATH="${PYCL_DIR}/exps/ewc_c80to110_mem2000_resnet34.json"

if [ ! -f "${CONFIG_PATH}" ]; then
  echo "[error] missing config: ${CONFIG_PATH}"
  echo "        请先确认已生成配置文件 exps/ewc_c80to110_mem2000_resnet34.json"
  exit 1
fi

echo "[Run] PyCIL EWC with --config=${CONFIG_PATH}"
(
  cd "${PYCL_DIR}"
...
LOG_DIR="${PYCL_DIR}/logs/ewc/cifar110/80/5/reproduce_1_resnet34"
IN_DS="CIFAR-110"
