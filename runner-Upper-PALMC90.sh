...
# save_path=checkpoints/${id}-${backbone}-${method}-upper-retain-c80.pt
save_path=checkpoints/${id}-${backbone}-${method}-upper-retain-c90.pt
...
# tag for outputs
# method_tag=${method}-upper-retain-c80
method_tag=${method}-upper-retain-c90
...
# 2) Optionally copy as a pretrain checkpoint for downstream use
# pretrain_ckpt=checkpoints/${id}-${backbone}-${method}-with-prototypes-c80.pt
pretrain_ckpt=checkpoints/${id}-${backbone}-${method}-with-prototypes-c90.pt
cp -f "$save_path" "$pretrain_ckpt" || true

# 3) Evaluate: use trained ckpt; retain(ID) vs standard OOD + forget(OOD)
score="mahalanobis"
bash eval.sh "$id" "$ood" "$backbone" "$method_tag" "$save_path" "$score" "$cache" "0" \
  "" "$forget_csv" "" "$forget_lambda" "" "" "" "" \
  --umap_enable --umap_rf_only "" "" ""
