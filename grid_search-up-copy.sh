...
tag_suffix="-epochgrid-lambda0.2"
...
train_and_eval() {
  ...
  local eval_output
  eval_output=$(bash eval.sh "$id" "$ood_joined" "$backbone" "$method_tag" "$pretrain_ckpt" mahalanobis "$cache" 0 "$adapter_path" "$forget_csv" "" "$flambda" "$lora_r" "$lora_alpha" "$lora_dropout" "$lora_target" --umap_enable --umap_rf_only "" 2>&1)
...
for fl in "${lambda_list[@]}"; do
  for lr in "${lr_list[@]}"; do
    for ep in "${epoch_list[@]}"; do
      ...
      echo "$ts,$id,$backbone,$method_tag,$ep,$lr,$fl,$batch,$wd,$cache,$k,$m,$use_peft,$lora_r,$lora_alpha,$lora_dropout,$lora_target,$batch_forget_mode,$temp,$adapter_path,$pretrain_ckpt,\"$ood_joined\",${eval_avg_fpr:-},${eval_avg_auroc:-},${eval_forget_fpr:-},${eval_forget_auroc:-},${eval_forget_auin:-},${eval_retain_acc:-},${eval_score:-}" >> "$results_csv"
    done
  done
done
